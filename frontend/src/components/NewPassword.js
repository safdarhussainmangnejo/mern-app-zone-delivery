...
function NewPassword() {
  ...
  useEffect(() => {
    // check if user is already loged in and if token is in localstorage then move to Home page, not login page
  ...
  const submitData = async (e) => {
    ...
    // calling new password API
    await axios
      .post(`http://localhost:5000/api/new-password`, {
        password: password,
        token: token,
      })
      .then((res) => {
